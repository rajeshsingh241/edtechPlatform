...
exports.sendotp = async (req, res) => {
  try {
    ...
    // Save OTP in DB first
    const otpBody = await OTP.create({ email, otp });
    console.log("DB ENTRY FOR OTP CREATED", otpBody);

    // Send OTP via mail
    try {
      await mailSender(email, "OTP Verification", `<h2>Your OTP is ${otp}</h2>`);
...
// Without populate → sirf ObjectId milega (like a reference / pointer).
//  With populate → wo ObjectId ko follow karke uske andar ka actual document (jaise name, description, price etc.) bhi le aata hai + saath me ObjectId bhi rehta hai.